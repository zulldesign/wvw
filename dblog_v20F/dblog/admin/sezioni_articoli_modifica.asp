...
<%
	If Request.QueryString("a") = "modifica" Then
		...
		SQLControllo = " SELECT TOP 1 ID FROM Articoli WHERE [Sezione] = '"& DoppioApice(FNomeVecchio) &"' "
		...
		If Errore = False Then
			SQLModifica = " UPDATE Articoli SET [Sezione] = '"& DoppioApice(FNomeNuovo) &"' WHERE [Sezione] = '"& DoppioApice(FNomeVecchio) &"' "
...
%>
									<p align="justify">
										...
										<br><a href="sezioni_articoli_elenco.asp"><%=Testo_Articoli_LinkTornaIndietro%></a></a>.
									</p>
<%
		Else
%>
									<p align="justify">
										...
										<br><a href="javascript:history.back(-1);"><%=Testo_Articoli_LinkTornaIndietro%></a></a>.
									</p>
<%
		End If
	Else
...
%>
									<form action="sezioni_articoli_modifica.asp?a=modifica" method="post">
