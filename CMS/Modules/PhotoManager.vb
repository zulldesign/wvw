...
Namespace WebApplication
  ...
  Public Module PhotoManager
    Function EnabledToMaintainOriginalImageSize(ByVal UserToCheck As User, ByVal Setting As SubSite) As Boolean
      Return UserToCheck.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior
    End Function
    ...
    Public MustInherit Class ImageBox
      Inherits System.Web.UI.UserControl
      Public MustOverride Sub SetImage(ByVal Photo As Photo, ByVal Setting As SubSite, Optional ByVal Size As Integer = 0)
    End Class
    ...
    Sub AddScriptClientPreviewPhoto(ByVal PhotoList As WebControl, ByVal ImagePreview As Control)
      ...
      PhotoList.Parent.Controls.Add(Components.Script(ScriptCode2, Components.ScriptLanguage.javascript))
    End Sub

    Sub AddScriptClientPopulateListPhotos(WhereSorceSelector As Control, PhotoList As WebControl, RadioButtonsText() As String, Checked As Integer, SelectedValue As String, DefaultJavascriptPhotosList As Integer, ParamArray JavascriptPhotosList() As String)
      ...
      PhotoList.Parent.Controls.Add(Components.Script(ScriptLoadSource, Components.ScriptLanguage.javascript))
      ...
      PhotoList.Parent.Controls.Add(Components.Script(ScriptPopulate, Components.ScriptLanguage.javascript))

      If Not RadioButtonsText Is Nothing Then
        Dim CodeLoad As String
        CodeLoad = "LoadSource(" & Checked & ");" & vbCrLf
        PhotoList.Parent.Controls.Add(Components.Script(CodeLoad, Components.ScriptLanguage.javascript))
      End If
    ...
    Public Sub SetPhotoList(ByVal PhotoList As Control, ByVal ImgPreview As Control, ByVal Setting As SubSite, ByVal TagPhotoSelect As String, ByVal DefaultJavascriptPhotosList As Integer, ByVal ParamArray JavascriptPhotosList() As String)
      ...
      AddScriptClientPopulateListPhotos(Nothing, CType(PhotoList, WebControl), Nothing, Nothing, TagPhotoSelect, DefaultJavascriptPhotosList, JavascriptPhotosList)

      'Add code preview photo
      AddScriptClientPreviewPhoto(CType(PhotoList, WebControl), ImgPreview)
    End Sub
    ...
    Function JavascriptPhotosList(ByVal Setting As SubSite, ByVal PhotoAlbum As String, Optional ByVal OnlyOfAuthor As String = Nothing, Optional ByVal AdditionalPhotoAlbum As String = Nothing) As String
      If Not String.IsNullOrEmpty(OnlyOfAuthor) Then
    ...
    Public Sub AddPhotos(ListPhotos As ListPhotos, Album As PhotoAlbum, Language As LanguageManager.Language, Optional IncludeSunfolder As Boolean = True, Optional OnlyThisAuthor As String = Nothing)
      'add name PhotoAlbum
      If Album IsNot Nothing Then
        ...
        For Each Photo As Photo In Album.PhotosSortedByTitle(Language)
          If OnlyThisAuthor = Nothing OrElse StrComp(Photo.Author, OnlyThisAuthor, CompareMethod.Text) = 0 Then
            Dim TextValue As New TextValue
            Dim Text As String = Photo.Title(Language)
            If Not String.IsNullOrEmpty(Photo.Description(Language)) Then
              Text &= " (" & TruncateText(Photo.Description(Language), 30) & ")"
            End If
            If String.IsNullOrEmpty(Text) Then
              Text = CStr(Photo.Name)
            End If
            TextValue.text = Text
    ...
    Private Function OperationAvailable(UserToCheck As Authentication.User, Setting As SubSite, Attributes As Attributes, Operation As Operations) As Boolean
      ...
      If UserToCheck.Role(Setting.Name) >= Authentication.User.RoleType.WebMaster Then
        Return True
      Else
        Select Case Operation
          Case Operations.View
            If Attributes.Hide = True Then
              Return EnableUserAtOperations(UserToCheck, Attributes.Author, Setting)
            End If
          ...
          Case Operations.Edit
            Select Case Attributes.Editable
              ...
              Case EditablePermission.OnlyIfEmpty
                If IsNothing(Attributes.Title) AndAlso IsNothing(Attributes.Description) Then
                  Return True
                End If
            End Select
      ...
      Return False
    End Function
    Public Function DescriptionOperation(Operation As Operations, TypeObject As TypeObject, Language As LanguageManager.Language) As String
      ...
      Return Nothing
    End Function
    Public Function ControlOperations(User As Authentication.User, Attributes As Attributes, Setting As SubSite, Optional ActualOperationState As Operations = Operations.View) As Control
      ...
      For Operation As Operations = Operations.View To LastOperation
        ...
        End If
      Next
      If CBool(Box.Controls.Count) Then
    ...
    Public Function AllPhotosName(Album As String) As Integer()
      ...
      If IO.Directory.Exists(Path) Then
        ...
        For Each File As String In Files
          Names(N) = ValInt(File.Substring(P))
          N += 1
        Next
    ...
    Private Function AllParents(PhotoAlbum As PhotoAlbum) As Collections.Generic.List(Of PhotoAlbum)
    ...
    Private Function AllPhotoAlbumsName(Album As String) As Array
      Dim Path As String = MapPath(PhotoAlbumsDirectory & "/" & Album)
      If IO.Directory.Exists(Path) Then
        Dim P As Integer = Len(Path) + 1
    ...
    Public Function Photos(Album As String) As Collections.Generic.Dictionary(Of Integer, Photo)
        ...
        If Files IsNot Nothing AndAlso Files.Length <> 0 Then
          ...
          For N = 1 To MaxQuantity
            If ExcludeName.Count < Files.Length Then
              Do
                Dim File As System.IO.FileInfo = Files(Random.Next(Files.Length))
                NamePhoto = ValInt(File.Name)
    ...
    Private Function PhotoAlbums(PhotoAlbum As PhotoAlbum, Optional IncludeParents As Boolean = True) As Collections.Generic.List(Of PhotoAlbum)
      ...
      If Not IsNothing(PAs) Then
        For Each Name As String In PAs
          SubPhotoAlbum = CType(PhotoAlbum.Load.GetItem(PhotoAlbum.Name & "/" & Name), PhotoManager.PhotoAlbum)
    ...
    Public Function AddPhoto(Image As System.Drawing.Image, Album As String, Optional Name As Integer = 0, Optional Size As Integer = 0, Optional Width As Integer = 0, Optional Height As Integer = 0) As Photo.ImageType
      ...
      Else
        If (Image.PixelFormat And Drawing.Imaging.PixelFormat.Indexed) <> 0 Then
    ...
    Private Function NewNamePhoto(Album As String) As Integer
      Dim Path As String = MapPath(PhotoAlbumsDirectory & "/" & Album & "/")
      Dim n As Integer = 0, Xfile As String
      Do
        n = n + 1
    ...
    Private Function NewNamePhotoAlbum(Album As String) As String
    ...
    Public Class PhotoAlbum
      ...
      Shared Function LoadXml(Name As String) As PhotoAlbum
        Name = ReplaceBin(Name, "\", "/") 'Rename for retrocompatibility
        Dim PhotoAlbum As PhotoAlbum = CType(Deserialize(NameFileSetting(Name), GetType(PhotoAlbum)), PhotoManager.PhotoAlbum)
      ...
      Public Function Control(Setting As Config.SubSite, Optional RelativeSubLevel As Integer = 0, Optional HrefCurrentLevel As Boolean = False) As Control
        Control = ControlPhotoAlbum(Me, Setting, RelativeSubLevel, HrefCurrentLevel)
      End Function

      Public Function ControlPhotos(Setting As Config.SubSite, Optional Target_Blank As Boolean = False, Optional DomainConfiguration As DomainConfiguration = Nothing, Optional HttpContext As HttpContext = Nothing) As Control
        ...
        For Each Photo As PhotoManager.Photo In Me.Photos.Values
          Dim CtrlPhoto As Control = Photo.ControlThumbnail(Setting, Target_Blank, Config.SizeThumbnail(), ThumbnailBoxMode.Gallery, DomainConfiguration, HttpContext)
        ...
        Return Box
      End Function
      Public Function PhotoAlbums(Optional IncludeParents As Boolean = True) As Collections.Generic.List(Of PhotoAlbum)
        PhotoAlbums = PhotoManager.PhotoAlbums(Me, IncludeParents)
      End Function
      Public Function Navigator(Setting As Config.SubSite, Optional HrefCurrentLevel As Boolean = False) As Control
      ...
      Public Property LocalizationArray As LiteralLanguage()
        Get
          ...
          Dim n As Integer = 0
          For Each Key As LanguageManager.Language In Localization.Keys
      ...
      Public Function Parent() As PhotoAlbum
        If IsRoot = False Then
          ...
          If CBool(p) Then
            Dim Album As String = Extension.Left(Me.Name, p - 1)
            Return CType(PhotoAlbum.Load.GetItem(Album), PhotoAlbum)
          End If
        End If
        Return Nothing
      End Function
      Public Property IsRoot() As Boolean
        Get
          If String.IsNullOrEmpty(Me.Name) OrElse Me.Name = DefaultPhotoAlbum Then
            IsRoot = True
          Else
            IsRoot = _IsRoot
          End If
      ...
      Sub Save()
        ...
        SaveXml()
        Load.SetItem(Name, Me)
        Pipeline.NotifyChangement(GetType(PhotoAlbum), Name)
      End Sub

      Function CreateSubFotoAlbum(Author As Authentication.User, Setting As SubSite) As PhotoAlbum
        If OperationAvailable(Author, Setting, Operations.CreateSubFotoAlbum) Then
          Return CreateSubFotoAlbum(Author)
        End If
        Return Nothing
      End Function

      Function CreateSubFotoAlbum(Optional ByVal Author As Authentication.User = Nothing) As PhotoAlbum
        ...
        If Author IsNot Nothing Then
          PhotoAlbum.Author = Author.Username
        End If
        PhotoAlbum.Save()
        Return PhotoAlbum
      End Function
      Sub Delete(User As Authentication.User, Setting As SubSite)
        If OperationAvailable(User, Setting, Operations.Delete) Then
          Dim PathPosition As String = MapPath(PhotoAlbumsDirectory & "/" & Me.Name)
          If PathPosition.Contains(":") Then    'controllo per la sicurezza assoluta di non eliminare directory importanti
            If CBool(Len(Config.ReadWriteDirectory)) Then      'controllo per la sicurezza assoluta di non eliminare directory importanti
              If CBool(Len(Config.PhotoAlbums_Sub_Directory)) Then      'controllo per la sicurezza assoluta di non eliminare directory importanti
                If CBool(InStr(PathPosition, ReplaceBin(Config.PhotoAlbums_Sub_Directory, "/", "\"), CompareMethod.Binary)) Then
        ...
        End If
      End Sub
      Sub Edit(User As Authentication.User, Setting As SubSite, Title As String, Description As String)
        ...
        End If
      End Sub
      Public Function Attributes(User As Authentication.User, Language As LanguageManager.Language) As Attributes
        ...
        Return Available
      End Function
      Public Function OperationAvailable(User As Authentication.User, Setting As SubSite, Operation As Operations) As Boolean
        Return PhotoManager.OperationAvailable(User, Setting, Me.Attributes(User, Setting.Language), Operation)
      End Function
      Private Function EnableUserAtOperations(UserToCheck As Authentication.User, Setting As SubSite) As Boolean
        Return PhotoManager.EnableUserAtOperations(UserToCheck, Me.Author, Setting)
      End Function
      Public Function ControlOperations(User As Authentication.User, Setting As SubSite, Optional ActualOperationState As Operations = Operations.View) As Control
    ...
    Public Class Photo
      ...
      Function Src(Setting As SubSite, Optional Size As Integer = 0, Optional ColorTone As ImageManager.ColorTone = ImageManager.ColorTone.Default) As String
        ...
        If Size = -1 Then
          ...
          Url &= "&fullsize=" & Size
        Else
          If CBool(Size) AndAlso Size <> Config.ShowDefaultImageSize(Nothing) Then
            Url &= "&size=" & Size
          End If
        End If
        If ColorTone <> ImageManager.ColorTone.Default Then
          Url &= "&tone=" & ColorTone
        End If
        Return Url
      End Function

      Function SrcThumbnail(Setting As SubSite, Optional Size As Integer = 0, Optional HttpContext As HttpContext = Nothing, Optional ColorTone As ImageManager.ColorTone = ImageManager.ColorTone.Default) As String
        If Size = 0 Then
          Size = Config.SizeThumbnail()
        End If
        'This is the most powerful of the StringBuilder, do not change!
        ...
        Return Url
      End Function
      Public Function Navigator(Setting As Config.SubSite, Optional HrefCurrentLevel As Boolean = True) As Control
        Return Me.PhotoAlbum().Navigator(Setting, HrefCurrentLevel)
      End Function
      Function PhotoAlbum() As PhotoManager.PhotoAlbum
        Return CType(PhotoManager.PhotoAlbum.Load.GetItem(Me.Album), PhotoManager.PhotoAlbum)
      End Function
      Public Function ControlThumbnail(Setting As Config.SubSite, Optional Target_Blank As Boolean = False, Optional Size As Integer = 0, Optional BoxMode As ThumbnailBoxMode = ThumbnailBoxMode.Thumbnail, Optional DomainConfiguration As DomainConfiguration = Nothing, Optional HttpContext As HttpContext = Nothing) As Control
        If DomainConfiguration Is Nothing Then DomainConfiguration = CurrentDomainConfiguration()
        Return PhotoManager.ControlThumbnail(Me, Setting, Target_Blank, Size, BoxMode, DomainConfiguration, HttpContext)
      End Function
      Public Function ControlPhoto(Setting As Config.SubSite) As Control
        ...
        If _Image Is Nothing Then
          Try
            ...
            If Width = 0 Then
              ...
              Me.Save()
            End If
            _Image = ImageManager.ApplyColorTone(_Image, Setting)
          Catch ex As Exception
            Extension.Log("LoadImageError", 100, Me.Album, Me.Name, NameFileImage, ex.Message, ex.Source, ex.StackTrace)
          End Try
        End If
        Return _Image
      End Function
      ...
      Sub FromStream(Stream As System.IO.Stream, Optional MaintainOriginal As Boolean = False)
      ...
      Sub FromUrl(Url As String, Optional MaintainOriginal As Boolean = False)
        MaintainOriginalFormat = MaintainOriginal
      ...
      Private Sub SetExif()
        If _Image IsNot Nothing Then
          ...
          'Geo
          Dim LatitudeA As UInt32 = 0, LatitudeB As UInt32 = 0, LatitudeC As UInt32 = 0, LatitudeD As UInt32 = 0, LatitudeE As UInt32 = 0, LatitudeF As UInt32 = 0
          Dim LatitudeRef As String = Nothing
          Dim LongitudeA As UInt32 = 0, LongitudeB As UInt32 = 0, LongitudeC As UInt32 = 0, LongitudeD As UInt32 = 0, LongitudeE As UInt32 = 0, LongitudeF As UInt32 = 0
          Dim LongitudeRef As String = Nothing

          Dim N32 As UInt32 = 0
          Dim N32B As UInt32 = 0
          For Each Id As Integer In _Image.PropertyIdList
            ...
            Try
              Select Case Id
                ...
                Case 2
                  ExifRational(PropertyItem, LatitudeA, LatitudeB, LatitudeC, LatitudeD, LatitudeE, LatitudeF)
                  Latitude = (LatitudeA / LatitudeB) + (LatitudeC / LatitudeD) / 60 + (LatitudeE / LatitudeF) / 3600.0
                Case 3
                  LongitudeRef = ExifText(PropertyItem)
                Case 4
                  ExifRational(PropertyItem, LongitudeA, LongitudeB, LongitudeC, LongitudeD, LongitudeE, LongitudeF)
                  Longitude = (LongitudeA / LongitudeB) + (LongitudeC / LongitudeD) / 60 + (LongitudeE / LongitudeF) / 3600.0
                Case 271
                ...
                Case 33434
                  ...
                  ExposureTime = CSng(N32 / N32B)
                Case 33437
                  ExifRational(PropertyItem, N32, N32B)
                  FNumber = CSng(N32 / N32B)
                Case &H8827
                  ISO = ExifN16(PropertyItem)
                Case 37381
                  ExifRational(PropertyItem, N32, N32B)
                  MaxAperture = CInt(N32 / N32B)
                Case 37385
                  Flash = ExifN16(PropertyItem)
                Case 37386
                  ExifRational(PropertyItem, N32, N32B)
                  FocalLength = CSng(N32 / N32B)
              End Select
            Catch ex As Exception
              'Exif error
            End Try
          Next
          If Not String.IsNullOrEmpty(LatitudeRef) Then
            If LatitudeRef = "S" Then
              Latitude = -Latitude
            End If
          End If
          If Not String.IsNullOrEmpty(LongitudeRef) Then
      ...
      End Function
      Private Sub ExifRational(PropertyItem As Drawing.Imaging.PropertyItem, ByRef N32 As UInt32, Optional ByRef N32B As UInt32 = 0, Optional ByRef N32C As UInt32 = 0, Optional ByRef N32D As UInt32 = 0, Optional ByRef N32E As UInt32 = 0, Optional ByRef N32F As UInt32 = 0)
        ...
        'End If
        If PropertyItem.Len > 4 Then
          N32B = Convert.ToUInt32(PropertyItem.Value(4)) Or Convert.ToUInt32(PropertyItem.Value(5)) << 8 Or Convert.ToUInt32(PropertyItem.Value(6)) << 16 Or Convert.ToUInt32(PropertyItem.Value(7)) << 24
        End If
        If PropertyItem.Len > 8 Then
          N32C = Convert.ToUInt32(PropertyItem.Value(8)) Or Convert.ToUInt32(PropertyItem.Value(9)) << 8 Or Convert.ToUInt32(PropertyItem.Value(10)) << 16 Or Convert.ToUInt32(PropertyItem.Value(11)) << 24
        End If
        If PropertyItem.Len > 12 Then
          N32D = Convert.ToUInt32(PropertyItem.Value(12)) Or Convert.ToUInt32(PropertyItem.Value(13)) << 8 Or Convert.ToUInt32(PropertyItem.Value(14)) << 16 Or Convert.ToUInt32(PropertyItem.Value(15)) << 24
        End If
        If PropertyItem.Len > 16 Then
          N32E = Convert.ToUInt32(PropertyItem.Value(16)) Or Convert.ToUInt32(PropertyItem.Value(17)) << 8 Or Convert.ToUInt32(PropertyItem.Value(18)) << 16 Or Convert.ToUInt32(PropertyItem.Value(19)) << 24
        End If
        If PropertyItem.Len >= 20 Then
          N32F = Convert.ToUInt32(PropertyItem.Value(20)) Or Convert.ToUInt32(PropertyItem.Value(21)) << 8 Or Convert.ToUInt32(PropertyItem.Value(22)) << 16 Or Convert.ToUInt32(PropertyItem.Value(23)) << 24
        End If
      End Sub
      ...
      Function ExifInfo(Setting As SubSite) As Control
        ...
        If Taken <> Date.MinValue Then
          ...
          Div.Controls.Add(BR)
        End If
        If Not String.IsNullOrEmpty(CameraBrand) Then
          Div.Controls.Add(TextControl(Phrase(Setting.Language, 324) & "=" & CameraBrand))
          Div.Controls.Add(BR)
        End If
        If Not String.IsNullOrEmpty(CameraModel) Then
          Div.Controls.Add(TextControl(Phrase(Setting.Language, 325) & "=" & CameraModel))
          Div.Controls.Add(BR)
        End If
        If ExposureTime <> 0.0 Then
          Dim Exposure As String = Nothing
          If ExposureTime < 1 Then
            Exposure = "1/" & CInt(1 / ExposureTime) & " "
          End If
          Dim SecString As String = ExposureTime.ToString("0.###", Globalization.CultureInfo.InvariantCulture) & " sec."
          If Not String.IsNullOrEmpty(Exposure) Then
            SecString = "(" & SecString & ")"
          End If
          Exposure &= SecString
        ...
        End If
        If FNumber <> 0.0 Then
          Div.Controls.Add(TextControl("F-number" & "=" & FNumber.ToString("0.##", Globalization.CultureInfo.InvariantCulture)))
          Div.Controls.Add(BR)
        End If
        If ISO <> 0 Then
          ...
          Div.Controls.Add(BR)
        End If
        If MaxAperture <> 0 Then
          Div.Controls.Add(TextControl("Max Aperture" & "=" & MaxAperture.ToString("0.##", Globalization.CultureInfo.InvariantCulture)))
          Div.Controls.Add(BR)
        End If
        If Flash <> 0 Then
        ...
        If FocalLength <> 0.0 Then
          Div.Controls.Add(TextControl(Phrase(Setting.Language, 327) & "=" & FocalLength.ToString("0.##", Globalization.CultureInfo.InvariantCulture) & " mm"))
          Div.Controls.Add(BR)
        End If
        If CBool(Latitude) OrElse CBool(Longitude) Then
          Div.Controls.Add(TextControl(Phrase(Setting.Language, 3031) & "="))
          Dim Link As New HyperLink
          Link.Text = Latitude.ToString("0.####", Globalization.CultureInfo.InvariantCulture) & " " & Longitude.ToString("0.####", Globalization.CultureInfo.InvariantCulture)
          Link.NavigateUrl = Href(Setting.Name, False, "earth.aspx", EarthManager.QueryStringParameters(Nothing, Nothing, CSng(Latitude), CSng(Longitude)))
      ...
      Shared Function LoadXml(ByVal Album As String, ByVal Name As Integer) As Photo
        ...
        If IO.File.Exists(File) Then
          Dim Photo As Photo = CType(Deserialize(File, GetType(Photo)), PhotoManager.Photo)
          Photo.Name = Name
          Photo.Album = Album
          Return Photo
        End If
      ...
      Sub Save(Optional Author As Authentication.User = Nothing)
        If String.IsNullOrEmpty(Me.Album) Then
        ...
        If Image_Is_Updated Then
          ...
          If IsNothing(Author) = False Then
            Me.Author = Author.Username
          End If
        End If
      ...
      Shared Function Load(Optional Name As Integer = 0, Optional Album As String = Nothing) As Photo
    ...
    Private Function EnableUserAtOperations(UserToCheck As Authentication.User, Author As String, Setting As SubSite) As Boolean
      If UserToCheck.Role(Setting.Name) > Authentication.User.RoleType.Visitors Then
        If UserToCheck.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior Then
          Return True
        ElseIf UserToCheck.Username = Author Then
          Return True
        End If
      End If
      Return False
    ...
    End Function
    Function HrefPhoto(ByVal PhotoAlbum As String, ByVal PhotoName As Integer, ByVal Setting As Config.SubSite, Optional ByVal Domain As DomainConfiguration = Nothing) As String
      If Domain Is Nothing Then Domain = CurrentDomainConfiguration()
    ...
    Private Function ControlPhotoAlbum(PhotoAlbum As PhotoAlbum, Setting As Config.SubSite, RelativeSubLevel As Integer, Optional HrefCurrentLevel As Boolean = False, Optional Domain As DomainConfiguration = Nothing) As Control
      Dim Href As String = Nothing, AddTx As String = Nothing
      If (PhotoAlbum.SubLevel <> RelativeSubLevel) OrElse HrefCurrentLevel Then
    ...
    Private Function ControlThumbnail(Photo As Photo, Setting As Config.SubSite, Optional Target_Blank As Boolean = False, Optional Size As Integer = 0, Optional BoxMode As ThumbnailBoxMode = ThumbnailBoxMode.Thumbnail, Optional DomainConfiguration As DomainConfiguration = Nothing, Optional HttpContext As HttpContext = Nothing) As Control
      If Size = 0 Then
        Size = Config.SizeThumbnail()
      End If
      Dim Width As Integer = 0, Height As Integer = 0
      If CBool(Photo.Width) Then
        ' Get width
        If Photo.Width > Photo.Height Then
          Width = Size
        Else
          Width = CInt(Size * Photo.Width / Photo.Height)
        End If
        ' Get height
        If Photo.Height > Photo.Width Then
          Height = Size
        Else
          Height = CInt(Size * Photo.Height / Photo.Width)
        End If
      End If
    ...
    Public Function ControlPhoto(Photo As Photo, Setting As Config.SubSite) As Control
      Dim Author As User = User.Load(Photo.Author)
      If Author IsNot Nothing Then
        CurrentMasterPage().AddAuthors(Author)
      End If
      Dim Width As Integer = 0, Height As Integer = 0
