...
Namespace WebApplication
  ...
  Public Module LanguageManager
    Public Acronyms() As String = {"--", "EN", "IT", "RU", "DE", "ES", "FR", "ZH", "JP", "AR", "KO", "PT", "GR", "TR", "LV", "NL", "CZ", "VN"}
    Public Enum Language As Integer
      ...
      Vietnamese
    End Enum
    Public LanguagesUsed As Language() = {Language.English, Language.Italian, Language.Russian, Language.Spanish, Language.French, Language.Dutch, Language.Chinese, Language.Portuguese, Language.Turkish, Language.Latvian, Language.Nederland, Language.Czech, Language.Vietnamese}
    Function Culture(Language As Language) As Globalization.CultureInfo
      ...
      ElseIf Language = LanguageManager.Language.Chinese Then
        Name = "zh-CN"
      ElseIf Language = LanguageManager.Language.Czech Then
        Name = "cs-CZ"
      ElseIf Language = LanguageManager.Language.Vietnamese Then
        Name = "vi-VN"
      Else
    ...
    Function LanguageDefinition(ByVal Language As Language, ByVal Output As Language) As String
      Return Phrase(Output, 79 + CInt(Language))
    End Function
    Public Sub SetLocalization(Control As Control, Language As Language)
      Dim Text As String
      ...
      For Each Ctrl In Control.Controls
        ...
        If Type.GetProperty("ToolTip") IsNot Nothing Then
          Text = Localize(CStr(CallByName(Ctrl, "ToolTip", CallType.Get)), Language)
          'Text = Localize(Ctrl.ToolTip, Language)'Option Explicit Off
          If Text IsNot Nothing Then
            CallByName(Ctrl, "ToolTip", CallType.Set, Text)
            'Ctrl.ToolTip = Text 'Option Explicit Off
          End If
        End If
        ...
        If Type.GetProperty("Text") IsNot Nothing Then
          If Type.IsVisible Then 'Without this verify get error in page "log.aspx" and "showphoto.aspx" wen app run un trust mode "Medium" under IIS 
            Text = Localize(CStr(CallByName(Ctrl, "Text", CallType.Get)), Language)
            'Text = Localize(Ctrl.Text, Language) 'Option Explicit Off
            If Text IsNot Nothing Then
              CallByName(Ctrl, "Text", CallType.Set, Text)
              'Ctrl.Text = Text 'Option Explicit Off
            End If
          End If
        End If

        If Type.GetProperty("ErrorMessage") IsNot Nothing Then
          Text = Localize(CStr(CallByName(Ctrl, "ErrorMessage", CallType.Get)), Language)
          'Text = Localize(Ctrl.ErrorMessage, Language) 'Option Explicit Off
          If Text IsNot Nothing Then
            CallByName(Ctrl, "ErrorMessage", CallType.Set, Text)
            'Ctrl.ErrorMessage = Text 'Option Explicit Off
          End If
        End If
        ...
        If Type.GetProperty("Value") IsNot Nothing Then
          If Not TypeOf Ctrl Is System.Web.UI.HtmlControls.HtmlInputFile Then 'Is Read only
            Text = Localize(CStr(CallByName(Ctrl, "Value", CallType.Get)), Language)
            'Text = Localize(Ctrl.Value, Language) 'Option Explicit Off
            If Text IsNot Nothing Then
              CallByName(Ctrl, "Value", CallType.Set, Text)
              'Ctrl.Value = Text 'Option Explicit Off
            End If
          End If
        End If

        If CBool(CallByName(Ctrl, "HasControls", CallType.Get)) Then
          'If Ctrl.HasControls Then 'Option Explicit Off
          SetLocalization(CType(Ctrl, UI.Control), Language)
          'SetLocalization(Ctrl, Language) 'Option Explicit Off
        End If
      Next
    End Sub

    Function Localize(ByRef Variable As String, ByRef Language As Language) As String
      If CBool(Variable.Length) Then
        If Variable.Chars(0) = "#"c Then
          If Char.IsNumber(Variable.Chars(1)) Then
            If Variable.IndexOf("#"c, 1) = -1 Then
              Dim n As Integer = CInt(Variable.Substring(1))
    ...
    Function XmlPhraseBooksNameFile(Optional ByVal NamePhraseBooks As String = "General") As String
      Return FileManager.MapPath(ReadWriteDirectory & "/" & NamePhraseBooks & EndFileName)
    End Function

    Function AllXmlPhraseBooksNameFiles() As String()
      Return System.IO.Directory.GetFiles(FileManager.MapPath(ReadWriteDirectory), "*" & EndFileName)
    End Function
    ...
    Property XmlPhraseBooks(ByVal NameFile As String) As Collections.Generic.Dictionary(Of Language, Collections.Generic.Dictionary(Of Integer, String))
      ...
      Get
        Dim Lemmas As System.Collections.Generic.List(Of Lemma) = CType(Deserialize(NameFile, GetType(System.Collections.Generic.List(Of Lemma))), System.Collections.Generic.List(Of WebApplication.LanguageManager.Lemma))
    ...
    Sub LoadPhrasesBooks()
      ...
      Else
        ...
        While ObjDataReader.Read()
          ID = CInt(ObjDataReader("id"))
          For Each Language As Language In LanguagesUsed
            Try
              TXT = CStr(ObjDataReader(Acronym(Language)))
            Catch ex As Exception
              TXT = ""
              If Language <> LanguageManager.Language.English Then
                TXT = Phrase(Language.English, ID)
                If String.IsNullOrEmpty(TXT) AndAlso Language <> LanguageManager.Language.Italian Then
      ...
      If LanguageManager.PhraseBooks IsNot Nothing Then
      ...
      For Each File In AllXmlPhraseBooksNameFiles()
        If File <> XmlFile Then
          Dim NamePhraseBooks As String = Extension.Left(File, File.Length - Len(EndFileName))
    ...
    Public Function Phrase(Language As Language, ID As Integer) As String
      Try
    ...
    Public Function Ask(ByVal Text As String, ByVal Language As Language) As String
      If Language = LanguageManager.Language.Spanish Then
        Ask = "¿" & Text & "?"
      ElseIf Language = LanguageManager.Language.Chinese Then
        Ask = Text & "？"
      Else
    ...
    Sub AddMultilingualSubSite(ArchiveId As Integer)
      SyncLock MultiLingualSite
        ...
        For Each Setup As Config.SubSite In Config.AllSubSite()
          If SubSiteToDomain.ContainsKey(Setup.Name) Then
            If Setup.Archive IsNot Nothing AndAlso Setup.Archive.Length = 1 Then
              If Setup.Archive(0) = ArchiveId AndAlso CBool(Setup.Language) Then
                If Not LanguageCollection.Contains(Setup.Language) Then
                  LanguageCollection.Add(Setup.Language)
                  ListOfSubsite.Add(Setup.Name)
                End If
              End If
            End If
          End If
    ...
    Sub DetectAllMultilingualSubSite()
      SyncLock MultiLingualSite
        ...
        For Each Setup As Config.SubSite In AllSubSite()
          If SubSiteToDomain.ContainsKey(Setup.Name) Then
            If Setup.Archive IsNot Nothing AndAlso Setup.Archive.Length = 1 Then
              If Not MultiLingualSite.ContainsKey(Setup.Archive(0)) Then
                AddMultilingualSubSite(Setup.Archive(0))
              End If
            End If
          End If
        Next
