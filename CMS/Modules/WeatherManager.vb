...
Namespace WebApplication
    ...
    End Function
      ...
      Public Condition As ConditionCodes = ConditionCodes.not_available
			...
			Public Sub New(ByVal WeatherCode As String)
				...
				Else
                  ...
                  Pressure = CSng(Node.Attributes("pressure").InnerText)
                  RelativeHumidity = CSng(Node.Attributes("humidity").InnerText)
                  Visibility = CInt(Node.Attributes("visibility").InnerText)
								Case "yweather:wind"
                  Wind = CInt(Node.Attributes("speed").InnerText)
                  WindDegrees = CInt(Node.Attributes("direction").InnerText)
							End Select
						Next
						...
						For Each Node As System.Xml.XmlNode In Item
							Select Case Node.Name
								Case "geo:lat"
                  Latitude = Convert.ToSingle(Node.InnerText, Globalization.CultureInfo.InvariantCulture)
								Case "geo:long"
                  Longitude = Convert.ToSingle(Node.InnerText, Globalization.CultureInfo.InvariantCulture)
								Case "yweather:condition"
                  Temperature = CSng(Node.Attributes("temp").InnerText)
                  Condition = CType(Integer.Parse(Node.Attributes("code").InnerText), Weather.ConditionCodes)
							End Select
						Next

            Dim GTM As Integer = CInt(Latitude / 15)
		...
		Sub UpdateWeather()
          ...
          If ValInt(WeatherCode).ToString = WeatherCode Then
            Parameter = "w="
          Else
            Parameter = "p="
          End If
      ...
      For Each EachSubSite As SubSite In AllSubSite()
        If Not EachSubSite.Weathers Is Nothing Then
          For Each WeatherLocality As WeatherLocality In EachSubSite.Weathers
