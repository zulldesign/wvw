...
Namespace WebApplication
	...
	Public Module ImageManager
		...
		Public Sub ImageResize(ByRef Image As System.Drawing.Image, ByRef MexSize As Integer, Optional ByVal NotIncrease As Boolean = False)
			...
			If Image.Width > Image.Height Then
				Width = MexSize
			Else
        Width = CInt(MexSize * Image.Width / Image.Height)
			End If
			...
			If Image.Height > Image.Width Then
				Height = MexSize
			Else
        Height = CInt(MexSize * Image.Height / Image.Width)
			End If

			Dim NewImage As Drawing.Image
      If NotIncrease AndAlso (Width > Image.Width OrElse Height > Image.Height) Then
        'Return = Image
      Else
    ...
    Public Function ImageRotate(ByRef Image As Bitmap, ByRef Angle As Single, Optional Resize As Boolean = True) As Bitmap
      If Resize Then
    ...
    Public Function ConvertVintageTone(Image As Drawing.Bitmap) As Drawing.Bitmap
      ...
      New Single() {0.0F, 0.0F, 0.0F, 1.0F, 0.0F}, _
      New Single() {0.1F, 0.0F, 0.0F, 0.0F, 1.0F}}
      Dim ColorMatrix As New Drawing.Imaging.ColorMatrix(ColorMatrixElements)
      Return ApplyColorMatrix(Image, ColorMatrix)
    End Function
    ...
    Public Function ApplyColorTone(Image As Drawing.Image, ColorTone As ColorTone, Setting As SubSite) As Drawing.Image
      If ColorTone <> ColorTone.None Then
        ...
        Select Case ColorTone
          Case ColorTone.AlCapone
            Image = ConvertAlCaponeTone(CType(Image, Bitmap))
          Case ColorTone.Cool
            Image = ConvertCoolTone(CType(Image, Bitmap))
          Case ColorTone.BlackAndWhite
            Image = ConvertBlackAndWhite(CType(Image, Bitmap))
          Case ColorTone.Ozono
            Image = ConvertOzonoTone(CType(Image, Bitmap))
          Case ColorTone.Polaroid
            Image = ConvertPolaroidTone(CType(Image, Bitmap))
          Case ColorTone.RayBan
            Image = ConvertRayBanTone(CType(Image, Bitmap))
          Case ColorTone.Sepia
            Image = ConvertSepiaTone(CType(Image, Bitmap))
          Case ColorTone.Trendy
            Image = ConvertTrendyTone(CType(Image, Bitmap))
          Case ColorTone.Vintage
            Image = ConvertVintageTone(CType(Image, Bitmap))
        End Select
      End If
      Return Image
    End Function

    Public Function ApplyColorTone(Image As Drawing.Image, Setting As SubSite) As Drawing.Image
