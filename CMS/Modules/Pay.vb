...
Namespace WebApplication
    ...
    End Function
      ...
      If Not String.IsNullOrEmpty(ServiceToPay) Then
      ...
      End If
      If Not String.IsNullOrEmpty(NotifyUrl) Then  'For IPN notification
        NotifyUrl = "&notify_url=" & HttpUtility.UrlEncode(NotifyUrl)
      End If
      Dim Lc As String = LanguageManager.Acronym(Language)
      Return "https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=" & HttpUtility.UrlEncode(PayToAccount) & "&currency_code=" & Currency & "&amount=" & HttpUtility.UrlEncode(Amount.ToString(Culture)) & "&item_name=" & HttpUtility.UrlEncode(DetailDescription) & IfStr(Not String.IsNullOrEmpty(DetailText), ": ", "") & HttpUtility.UrlEncode(DetailText) & "&lc=" & Lc & ReturnUrl & NotifyUrl & ItemNumber & "&button_subtype=services&no_note=1&no_shipping=1&bn=PP%2dBuyNowBF%3abtn_buynowCC_LG%2egif%3aNonHosted&charset=utf-8"
		End Function
      ...
      If Amount <= 10 AndAlso Not String.IsNullOrEmpty(Setup.PaymentGateway.PayPalAccountMicropayments) Then
    ...
    End Function
      ...
      If Not String.IsNullOrEmpty(Plugin) Then
        ...
        If PluginObject IsNot Nothing Then
          ...
          PluginObject.RaiseDelivery(IdOrder, EmailToVerify, Successful)
          Return Successful
        End If
      Else
        Try
              ...
              If LCase(Request.Form("receiver_Email")) = LCase(Setup.PaymentGateway.PayPalAccount) OrElse LCase(Request.Form("receiver_Email")) = LCase(Setup.PaymentGateway.PayPalAccountMicropayments) OrElse Request.Form("receiver_id") = Setup.PaymentGateway.PayPalAccount OrElse Request.Form("receiver_id") = Setup.PaymentGateway.PayPalAccountMicropayments Then
                Select Case Plugin
                  Case "newuserauthentication"
                    Dim UserAuthenticated As Authentication.User = Authentication.User.Load(IdOrder)
                    If UserAuthenticated.GeneralRole = Authentication.User.RoleType.User Then
                      UserAuthenticated.GeneralRole = Authentication.User.RoleType.Senior
                      UserAuthenticated.Attribute("FirstName") = Request.Form("first_name")
                      UserAuthenticated.Attribute("LastName") = Request.Form("last_name")
                      UserAuthenticated.Save()
                      Extension.Log("AuthenticationUser", 1000, UserAuthenticated.Username, Request.Form("first_name"), Request.Form("last_name"), Request.Form("payer_Email"), Request.Form("address_zip"), Request.Form("residence_country"))
                      Return True
                    Else
                      Extension.Log("PaymentError", 1000, Plugin, IdOrder, UserAuthenticated.GeneralRole & " RoleType<>1")
                    End If
                  Case Else
