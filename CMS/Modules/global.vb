  ...
  Public Class [Global]
    ...
    Sub Application_End(ByVal sender As Object, ByVal e As EventArgs)
      ...
      Catch ex As Exception
        Extension.Log("application_end", 1000, ex.Message & vbCrLf, ex.Source & vbCrLf, ex.StackTrace & vbCrLf, vbCrLf)
      End Try

      Try
        Dim LastError As System.Exception = Server.GetLastError.GetBaseException
        Extension.Log("application_end", 1000, LastError.Message & vbCrLf, LastError.Source & vbCrLf, LastError.StackTrace & vbCrLf, vbCrLf)
      Catch ex As Exception
        'No last error
      End Try
      ...
      Catch ex As Exception
        Extension.Log("application_end", 1000, ex.Message & vbCrLf, ex.Source & vbCrLf, ex.StackTrace & vbCrLf, vbCrLf)
      End Try
      Extension.Log("application_end", 1000, "successful")
    ...
    Sub Session_Start(ByVal sender As Object, ByVal e As EventArgs)
      ...
      If Setup.Performance.InhibitSessionForCrawlers AndAlso Extension.IsCrawler(HttpContext.Current.Request) Then
      ...
      Dim CounterAccess As Integer = ValInt(Cookie("CounterAccess"))
      CounterAccess += 1
      Session("CounterAccess") = CounterAccess
      Cookie("CounterAccess") = CStr(CounterAccess)
    End Sub
    ...
    Sub Session_End(ByVal sender As Object, ByVal e As EventArgs)
      ...
      If Setup.Performance.InhibitSessionForCrawlers AndAlso Me.Session("NoSession") IsNot Nothing Then
        Exit Sub
      End If
    ...
    Private Sub Global_BeginRequest(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.BeginRequest
      ...
      If Not String.IsNullOrEmpty(HttpContext.Current.Request.QueryString("ss")) AndAlso CurrentDomainConfiguration.SubSitesAvailableLength() = 1 Then
    ...
    Private Sub Global_Error(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Error
      ...
      Try
        ...
        'ThisError = Server.GetLastError.GetBaseException
          ...
          Select Case StatusCode
            ...
            Case 404
              ...
              If Not ExecutionFile.StartsWith("~/" & Resources) OrElse ExecutionFile.EndsWith(".aspx") Then
                Dim P As Integer = InStrRev(ExecutionFile, "/", -1, CompareMethod.Binary)
                If P > 2 Then
                  Dim ApplicationPath As String = HttpContext.Current.Request.ApplicationPath
                  If Not ApplicationPath.Last = "/"c Then
                    ApplicationPath &= "/"
                  End If
                  ...
                  Extension.Log("redirect", 1000, AbsoluteUri(HttpContext.Current.Request), Redirect, HttpContext.Current.Request.UserHostAddress, HostUser())
        ...
        Try
          HResult = Runtime.InteropServices.Marshal.GetHRForException(ThisError)
        Catch ex As Exception
        End Try
        Dim HexHResult As String = Hex(HResult)

        If Setup Is Nothing OrElse StatusCode <> 404 OrElse Setup.ReportErrorsByEmail.ExcludeErrorHttp404 = False Then
          If Setup Is Nothing OrElse HexHResult <> "80004005" OrElse Setup.ReportErrorsByEmail.ExcludeError80004005 = False Then
            If Setup Is Nothing OrElse HexHResult <> "80070057" OrElse Setup.ReportErrorsByEmail.ExcludeError80070057 = False Then
              If Setup Is Nothing OrElse HexHResult <> "800704CD" OrElse Setup.ReportErrorsByEmail.ExcludeError800704CD = False Then
                Common.ReportErrorByEmail(ThisError)
              End If
            End If
          End If
        End If

        Select Case HexHResult
          Case "8007000E" 'Out of memory
        ...
        If StatusCode = 500 Then
          Extension.Log("Error " & HexHResult, 1000, vbCrLf & ThisError.Message & vbCrLf, ThisError.Source & vbCrLf, HttpContext.Current.Request.Url.ToString & Reference & vbCrLf, HttpContext.Current.Request.UserAgent & vbCrLf, ThisError.StackTrace & vbCrLf, vbCrLf)
        Else
          Extension.Log("ErrorHttp " & StatusCode, 1000, vbCrLf & ThisError.Message & vbCrLf, ThisError.Source & vbCrLf, HttpContext.Current.Request.Url.ToString & Reference & vbCrLf, HttpContext.Current.Request.UserAgent & vbCrLf, ThisError.StackTrace & vbCrLf, vbCrLf)
        End If
      ...
      Catch ex As Exception
        Try
          Extension.Log("Error in Global_Error", 0, ex, ex.Message, ex.StackTrace)
