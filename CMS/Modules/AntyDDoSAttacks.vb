...
Namespace WebApplication
    ...
    Public Sub AntiAbnormalSimultaneousRequests(Request As HttpRequest)
      ...
      SyncLock IpDDoSAttacks
        ...
        'remowe thhis++++++++++
        'Static Record As Integer = 0 'rIMUOVERE QUESTA LINEA
        ...
        '      ADDED.Add(IP)
        '      Extension.Log("RecordDDoS", 1000, Counter, IP, Request.UserAgent, HostUser, AbsoluteUri(Request))
        '    End If
        '  End SyncLock
        'End If
        '+++++++++++++++++++
        ...
        If Counter > Max Then
          ...
          End SyncLock
          If Config.Setup.Security.DdosPrevention.LogEvents Then
            Dim UrlReferrer As String = Nothing : If Request.UrlReferrer IsNot Nothing Then UrlReferrer = Request.UrlReferrer.AbsoluteUri
            Extension.Log("DDoS_AntiAbnormalSimultaneousRequests", 1000, Counter.ToString, IP, Request.UserAgent, HostUser, AbsoluteUri(Request), UrlReferrer)
          End If
          StopResponse()
    ...
    Sub TemporarilyCrawlersStoppingDuringOverload(Request As HttpRequest)
      If Extension.IsCrawler(Request) Then
        If MonitorOfResources.AddCurrentThreadToQueue Then
          If Config.Setup.Security.DdosPrevention.LogEvents Then
            Dim UrlReferrer As String = Nothing : If Request.UrlReferrer IsNot Nothing Then UrlReferrer = Request.UrlReferrer.AbsoluteUri
            Extension.Log("DDoS_TemporarilyCrawlersStoppingDuringOverload", 1000, Request.UserHostAddress, Request.UserAgent, HostUser, AbsoluteUri(Request), UrlReferrer)
          End If
          StopResponse()
        End If
      End If
    ...
    Public Sub DisableTheSourcesOfAbnormalHighTraffic(Request As HttpRequest)
      Try
        ...
        Else
          ...
          Dim MaxRequest As Integer = 0
          Dim LimitRequestSlow As Integer = 0
                ...
                Else
                  ...
                  If IpHit = Hits.IP Then
                    ...
                    If Counter > MaxRequest Then
                      ...
                      If Config.Setup.Security.DdosPrevention.LogEvents Then
                        Dim UrlReferrer As String = Nothing : If Request.UrlReferrer IsNot Nothing Then UrlReferrer = Request.UrlReferrer.AbsoluteUri
                        Extension.Log("DDoS_DisableTheSourcesOfAbnormalHighTraffic", 1000, Counter.ToString, IpHit, Request.UserAgent, HostUser, AbsoluteUri(Request), UrlReferrer)
                      End If
                      StopResponse()
    ...
    Private Sub StopResponse()
      ...
      Response.Clear()
      If Config.Setup.SEO.ErrorToBeUsedInCaseOfOverload = Configuration.SearchEngineOptimizationConfiguration.HttpError.Error503 Then
