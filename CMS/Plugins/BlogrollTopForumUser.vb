...
Namespace WebApplication.Plugin		'Standard namespace obbligatory for all plugins
    ...
    Private Shared SharedConfiguration As BlogrollTopForumUserSharedConfiguration = CType(Plugin.LoadSharedConfiguration(), BlogrollTopForumUserSharedConfiguration)
    ...
    Private Shared Sub Plugin_MasterPagePreRender(ByVal MasterPage As Components.MasterPageEnhanced) Handles Plugin.MasterPagePreRender
      ...
      If HttpContext.Current.Request.Path.ToLower.EndsWith("/default.aspx") Then
        ...
        Else
          Configuration = CType(Plugin.LoadConfiguration(), BlogrollTopForumUserConfiguration)
        End If
        Dim DateLimit As Date = Today.AddDays(-Configuration.DaysToBeConsideredForTheRanking)
        If Not Extension.IsCrawler() OrElse Configuration.ShowToCrawler Then
          If Not HttpContext.Current.Request.Browser.IsMobileDevice OrElse Configuration.ShowToMobileDevice Then
            ...
            End If
            Th0.Attributes("colspan") = CStr(ColSpan)

            Dim Row As Integer = 1
            For Each UserInTop As Ranking.Day.ActivityOfUser In TopUserForThisSubsite
              Dim User As User = Authentication.User.Load(UserInTop.User)
              If User IsNot Nothing Then
                ...
                If CBool(ColWebsite) Then
                  Table.Rows(Row).Cells.Add(New HtmlTableCell)
                  If User.URL <> "" Then

                    Dim Verified As Boolean = True
                    If Configuration.ExcludeWebsiteWithCensoredWords Then
                      Dim WebsiteValid As String = CStr(User.Attribute("WebsiteWithCensoredWords"))
                    ...
                    If Verified Then
                      ...
                      Select Case Configuration.ShowWebsiteOfUsers
                        ...
                        Case Else
                          Dim TitleOfWebSite As String = CStr(User.Attribute("TitleOfWebSite"))
                          If TitleOfWebSite = "" OrElse CStr(User.Attribute("UrlOfWebSite")) <> User.URL Then
                            Try
                              TitleOfWebSite = New MetaTags(ReadHtmlFromWeb(User.URL)).Title
                            Catch ex As Exception
                            End Try
                ...
                If CBool(ColScore) Then
                  ...
                  Select Case Configuration.CriterionForRanking
                    Case BlogrollTopForumUserConfiguration.Criterion.TextLengthWrited
                      Table.Rows(Row).Cells(ColScore).InnerText = CStr(UserInTop.TextLengthWrited)
                    Case Else
                      Table.Rows(Row).Cells(ColScore).InnerText = CStr(UserInTop.PostWrited)
                  End Select
                End If
    ...
    End Sub
      ...
      Dim UsersForumRanking As Ranking = CType(Plugin.LoadObject(GetType(Ranking)), Ranking)
      ...
      If SortBy = BlogrollTopForumUserConfiguration.Criterion.TextLengthWrited Then
        Array.Sort(Result, New SortByTextLengthWrited)
      Else
        Array.Sort(Result, New SortByPostWrited)
      End If
      If Result.Length > MaxElements Then
      ...
      Implements IComparer(Of Ranking.Day.ActivityOfUser)
      Public Function Compare(ByVal ActivityOfUser1 As Ranking.Day.ActivityOfUser, ByVal ActivityOfUser2 As Ranking.Day.ActivityOfUser) As Integer Implements IComparer(Of Ranking.Day.ActivityOfUser).Compare
      ...
      End Function
		...
		Private Shared Sub Plugin_ForumReplyPosted(Forum As ForumManager.Forum, Reply As ForumManager.Reply, ForumPostType As PluginManager.ForumPostType, Setting As Config.SubSite) Handles Plugin.ForumReplyPosted
          ...
          Dim UsersForumRanking As Ranking = CType(Plugin.LoadObject(GetType(Ranking), Setting.Name), Ranking)
					SyncLock Plugin
						Dim TodayRank As Ranking.Day
            If UsersForumRanking.Days.Length = 0 OrElse UsersForumRanking.Days(0).Date <> Today Then
              TodayRank = New Ranking.Day
              TodayRank.Date = Today
              ReDim Preserve UsersForumRanking.Days(UsersForumRanking.Days.Length)
              UsersForumRanking.Days(UBound(UsersForumRanking.Days)) = TodayRank
              Dim SortCriterion As IComparer(Of Ranking.Day) = New SortByDate
      ...
      Implements IComparer(Of Ranking.Day)
      Public Function Compare(ByVal Day1 As Ranking.Day, ByVal Day2 As Ranking.Day) As Integer Implements IComparer(Of Ranking.Day).Compare
