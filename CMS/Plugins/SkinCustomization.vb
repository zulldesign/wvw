...
Namespace WebApplication.Plugin		'Standard namespace obbligatory for all plugins
    ...
    Private Shared Sub Plugin_GetSkinSetup(ByRef SetSkinSetup As Config.SkinSetup, Setting As Config.SubSite) Handles Plugin.GetSkinSetup
      'Customize SkinSetup
      Dim Configuration As SkinCustomizationConfiguration = CType(Plugin.LoadConfiguration(), SkinCustomizationConfiguration)
      SetSkinSetup = Config.SkinSetup.Load(CurrentSetting.Skin().Setups)
      For Each Key As String In SetSkinSetup.Variables.AllKeys
        SetSkinSetup.Variables(Key) = CStr(Configuration.SetupSkinParameters(Key))
      Next
    End Sub
      ...
      Dim Conf As SkinCustomizationConfiguration = CType(Configuration, SkinCustomizationConfiguration) 'Unboxing
