...
Namespace WebApplication.Plugin    'Standard namespace obbligatory for all plugins
  Public Class BackgroundSlideshow
    ...
    Private Shared Sub Plugin_BeforeEditPluginConfiguration(Configuration As Object, SetTablePropertyPhraseCorrispondence As StringDictionary) Handles Plugin.BeforeEditPluginConfiguration
      Dim BackgroundConfig As BackgroundSlideshowConfiguration = CType(Configuration, BackgroundSlideshowConfiguration) : Dim Setting As SubSite = CurrentSetting()
      If Setting.Photoalbums IsNot Nothing AndAlso Setting.Photoalbums.Count > 0 Then BackgroundConfig.IdPhotoAlbum = Setting.Photoalbums(0)
    End Sub
    ...
    Private Shared Sub Plugin_BeforeSavePluginConfiguration(Configuration As Object, ByRef InvokeConfigurationPageRefresh As Boolean) Handles Plugin.BeforeSavePluginConfiguration
      Dim BackgroundConfig As BackgroundSlideshowConfiguration = CType(Configuration, BackgroundSlideshowConfiguration)
      If BackgroundConfig.IntervalInSeconds < 10 Then BackgroundConfig.IntervalInSeconds = 10 'Don't allow setting a value less of 10 seconds
    End Sub
    ...
    Private Shared Sub Plugin_MasterPagePreRender(ByVal MasterPage As Components.MasterPageEnhanced) Handles Plugin.MasterPagePreRender
      Dim Configuration As BackgroundSlideshowConfiguration = CType(Plugin.LoadConfiguration(), BackgroundSlideshowConfiguration)
      If Configuration.OnlyInHomePage = False OrElse (StrComp(System.IO.Path.GetFileName(HttpContext.Current.Request.Path), "default.aspx", CompareMethod.Text) = 0 AndAlso HttpContext.Current.Request.QueryString(QueryKey.Show) = "" AndAlso HttpContext.Current.Request.QueryString(QueryKey.Url) = "" AndAlso HttpContext.Current.Request.QueryString(QueryKey.CryptedUrl) = "" AndAlso HttpContext.Current.Request.QueryString(QueryKey.ArticleNumber) = "") Then
        If HttpContext.Current.Request.Browser.EcmaScriptVersion.Major >= 1 Then 'Verify if browser support javascript
          If Configuration.IdPhotoAlbum <> "" Then
            ...
            Dim AllPhotos As Collections.Generic.List(Of Photo) = GetRandomPhotos(Configuration.IdPhotoAlbum, NSlide)
            If AllPhotos IsNot Nothing AndAlso AllPhotos.Count > 0 Then
              ...
              MasterPage.Bottom.Controls.Add(Components.Script(ScriptCode, ScriptLanguage.javascript))
            End If
          End If
        End If
      End If
