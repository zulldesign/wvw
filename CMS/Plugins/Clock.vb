...
Namespace WebApplication.Plugin		'Standard namespace obbligatory for all plugins
    ...
    Private Shared Sub Plugin_MasterPagePreRender(ByVal MasterPage As Components.MasterPageEnhanced) Handles Plugin.MasterPagePreRender
      Dim Configuration As ClockConfiguration = CType(Plugin.LoadConfiguration(), ClockConfiguration)
    ...
    End Sub
        ...
        Dim TextWriter As String = Nothing
				If Label <> "" Then
          Dim FontSize As Integer = CInt(Size / 10)
          TextWriter = ";var ct=c.canvas.getContext('2d');ct.font='bold " & FontSize & "px courier';ct.fillText('" & Label & "', " & -CInt(FontSize * Label.Length / 32 * 10) & ", " & (Size / 5) & ");"
				End If
        If CBool(TimeZone) Then
