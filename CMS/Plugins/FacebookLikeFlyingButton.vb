...
Namespace WebApplication.Plugin		'Standard namespace obbligatory for all plugins
    ...
    Private Shared Sub Plugin_MasterPagePreRender(ByVal MasterPage As Components.MasterPageEnhanced) Handles Plugin.MasterPagePreRender
      Dim Configuration As FacebookLikeFlyingButtonConfiguration = CType(Plugin.LoadConfiguration(), FacebookLikeFlyingButtonConfiguration)
      If HttpContext.Current.Request.Browser.EcmaScriptVersion.Major >= 1 Then 'Verify if browser support javascript
        If Not Extension.IsCrawler() OrElse Configuration.ShowToCrawler Then
          If CurrentUser(MasterPage.Session).GeneralRole = Authentication.User.RoleType.Visitors OrElse Configuration.HideToLoggedUser = False Then
            ...
            If (MetaTag Is Nothing OrElse MetaTag.Content <> "noindex") AndAlso MasterPage.ShowRightBar = True Then 'Share only page indexabled 
              MasterPage.Page.Header.Controls.Add(Components.Script("document.write('" & AbjustForJavascriptString(HtmlHeaderCode) & "')", ScriptLanguage.javascript))
              Dim Code As String = ReplaceBin(HtmlCodeInUsersToolsBar, CanonicalUrlNameVariable, HttpUtility.HtmlEncode(AbsoluteUri(HttpContext.Current.Request))) & ReplaceBin(FlyingScript, "speed", CStr(Configuration.Speed))
              MasterPage.UserToolsBar.Controls.AddAt(0, Components.Script("document.write('" & AbjustForJavascriptString(Code) & "')", ScriptLanguage.javascript))
            End If
          End If
        End If
      End If
