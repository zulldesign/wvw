...
Namespace WebApplication.Plugin		'Standard namespace obbligatory for all plugins
    ...
    Private Shared SharedConfiguration As ChatRobotSharedConfiguration = CType(Plugin.LoadSharedConfiguration(), ChatRobotSharedConfiguration)
    Private Shared QuestionsByLanguage As Collections.Generic.Dictionary(Of LanguageManager.Language, Collections.Generic.List(Of Question))
        ...
        QuestionsByLanguage = New Collections.Generic.Dictionary(Of LanguageManager.Language, Collections.Generic.List(Of Question))
          ...
          For Each FileName As String In System.IO.Directory.GetFiles(Plugin.Directory(), "*." & Acronym(Language) & ".txt")
            ...
            If IO.File.Exists(FileName) Then
              ...
              For Each EachLine As String In Lines
                Dim Line As String = EachLine 'necessary for compatibility with c# version
                NLine += 1
                'Try
                Select Case Line
                  Case "!"
                    ...
                    If Not Line.StartsWith("/") Then
                      If Line <> "" Then
                        Select Case Line(0)
                          Case "+"c, "="c, "-"c
                            Dim Ambit As String = ""
                            Dim AmbitType As Char = Char.MinValue
                      ...
                      Else
                        ...
                        End If
                      End If
                      LastLineIsEmpty = String.IsNullOrEmpty(Line)
    ...
    End Function
		...
		Private Shared Function Description(ByVal Language As LanguageManager.Language, ByVal ShortDescription As Boolean) As String
          ...
          Return "Plugin che aggiunge un robot autorisponditore il quale simula un operatore e assiste i visitatori nel sito web. NOTA: Questo plugin necessita del plugin ""Chat"" abilitato Occorre preventivamente programmare il robot tramite un apposito linguaggio: " & CopyrightLink & "?p=82&a=26&t=instructionschatlanguage"
				Else
          Return "Plugin to the addition of a robot autoresponder that simulates an operator to assist the visitors in the website. NOTE: This plugin requires the plugin ""Chat"" enabled. It should advance to program the robot using a special language: " & CopyrightLink & "?p=82&a=26&t=instructionschatlanguage"
				End If
			End If
		End Function
    ...
    Private Shared Function GetConfiguration() As WebApplication.Plugin.ChatRobot.ChatRobotConfiguration
      If SharedConfiguration.UseSharedConfigurationToAllSettings Then
        Return SharedConfiguration.ChatRobotConfiguration
      Else
        Return CType(Plugin.LoadConfiguration(), ChatRobotConfiguration)
      End If
    End Function
            ...
            End If
              ...
              If Now.ToUniversalTime() >= Message.PlaningTime Then
                ...
                If ToUser IsNot Nothing Then 'Dont send if the user is exit
                  ChatManager.SendMessage(Message.Text, Message.Robot, Authentication.User.Load(Message.Robot) Is Nothing, Message.ChatRoom)
              ...
              End If
				...
				Catch ex As Exception
          Extension.Log("RobotChatError", 100, ex.Message & vbCrLf & ex.Source & vbCrLf & ex.StackTrace & vbCrLf & vbCrLf)
				End Try
      ...
      Dim BestStimation As Integer = 0
      Dim BestQuestion As Question = Nothing
			...
			Else
              ...
              If BestStimation >= 100 + Ask.Length Then
                Exit For
              End If
						End If
					End If
          ...
          If CBool(BestQuestion.IfThisAnswerAddAmbitToUser.Count) Then
    ...
    End Function
      ...
      Dim Result As String = SetVariables(Phrases(RandomIntNumber(Phrases.Count - 1)), UserInChat)
        ...
        If Result.Contains("SEARCH") Then
          Dim ResultsOutsideSearch As New StringCollection
          WebApplication.Search.Find(ResultsOutsideSearch, PhreseSearch)
          If CBool(ResultsOutsideSearch.Count) Then
            Dim SearchOut As String = "http://" & ResultsOutsideSearch(0)
            Result = ReplaceBin(Result, "SEARCH", SearchOut)
          End If
        End If

        If Result.Contains("INSIDE") Then
          Dim ResultsInsideSearch As New StringCollection
          WebApplication.Search.Find(ResultsInsideSearch, PhreseSearch, UserInChat.Domine)
          If CBool(ResultsInsideSearch.Count) Then
      ...
      If AmbitCompatible(UserInChat.Ambit, Question.Ambit, Question.NotAmbit) Then
        ...
        For Each MindAsk() As String In Question.Ask
          ...
          If MindAskJoin = AskJoin Then
            ...
            SetBestStimation(BestStimation, 100 + MindAsk.Length + Question.AdditionalImportance)
          End If

          If AskJoin.Contains(MindAskJoin) Then
            'The phrase is present
            SetBestStimation(BestStimation, 90 + MindAsk.Length + Question.AdditionalImportance)
          End If

          If Ask.Length >= 1 AndAlso MindAsk.Length >= 1 Then
            ...
            For Each WordMind As String In MindAsk
              ...
              Next
            Next
            If Equal = MindAsk.Length Then
              'Sono presenti tutte le parole
              SetBestStimation(BestStimation, 80 + MindAsk.Length + Question.AdditionalImportance)
            End If
          End If
        Next
      End If
    ...
    Private Shared Sub FixAmbitToQuestion(ByVal Language As LanguageManager.Language, ByVal Question As Question, ByVal AmbitType As Char, ByRef Ambit As String)
      If Ambit <> "" Then
        Select Case AmbitType
          ...
          Case "="c
            ...
            If Len(Ambit) >= 6 Then
              If Ambit(0) = "P"c AndAlso Char.IsNumber(Ambit(1)) AndAlso Char.IsNumber(Ambit(Ambit.Length - 1)) Then
                Ambit = Mid(Ambit, 2)
                Dim Parameters() As String = Split(Ambit, "A")
                Dim PageMonitor As PageMonitor = New PageMonitor
                PageMonitor.Page = CInt(Parameters(0))
                PageMonitor.Archive = CInt(Parameters(1))
    ...
    Private Shared Sub Plugin_MasterPagePreRender(MasterPage As MasterPageEnhanced) Handles Plugin.MasterPagePreRender
      ...
      If QuestionsByLanguage.ContainsKey(MasterPage.Setting.Language) Then
        ...
        If Session.SessionID <> Request.QueryString(QueryKey.ChatRoom) Then
          ...
          End If
          Dim AmbitUser As StringCollection = CType(Session("AmbitsChatRobot"), StringCollection)
          ...
          If Not Request.Path.EndsWith("/chat.aspx") Then
            If Request.Browser.EcmaScriptVersion.Major >= 1 Then
              ...
              If Configuration.RobotsUserName.Length > 0 Then
                If CBool(Configuration.RobotsUserName.Length) Then
                  Dim RobotUserName As String
                  If Session("ChatRobotUserName") IsNot Nothing Then
                    RobotUserName = CStr(Session("ChatRobotUserName"))
                    Session("ShowChatRobot") = CInt(Session("ShowChatRobot")) + 1
                  Else
                    'Get Random a robot
                    RobotUserName = Configuration.RobotsUserName(RandomIntNumber(Configuration.RobotsUserName.Length - 1))
                  ...
                  Dim ShowInviteAfterSeconds As Integer = 0
                  ...
                  If Session("UserChatInfo") Is Nothing Then
                    ...
                    If CInt(Session("ShowChatRobot")) <= Configuration.HowManyTimesShowInviteForSessionNavigation Then
                    ...
                    End If
                  End If
                  Dim ChatInviteCode As String = ControlToText(ChatManager.ChatInvite(RobotUserName, MasterPage.Setting, Title))
                  'Dim ScriptPopup As Control = Script(ChatManager.JavaScriptPopUp(ChatInviteCode), ScriptLanguage.javascript)
                  Dim ScriptPopup As Control = Components.Script(ChatManager.JavaScriptPopUp(ChatInviteCode, ShowInviteAfterSeconds), ScriptLanguage.javascript)
                  ...
                  MasterPage.AddButton(Phrase(MasterPage.Setting.Language, 2001, 3199), Nothing, Nothing, IconType.Help, Components.MasterPageEnhanced.TargetForButton.Self, False, "if (!document.getElementById('NameUserChat')){ChatWindowPopUp()}")
                End If
              End If
            End If
          ...
          Else
            ...
            If ChatRoom = "_" & Session.SessionID Then
              ...
              If Session("UserChatInfo") Is Nothing Then
                ...
                UserChatInfo.InvitedByRobotName = CStr(Session("ChatRobotUserName"))
                UserChatInfo.User = MasterPage.User 'Assign Current User
                UserChatInfo.Entity.Ambit = CType(Session("AmbitsChatRobot"), StringCollection)
                ...
                If MasterPage.Setting.Forums IsNot Nothing Then
                  ...
                  Next
                End If
                If UserChatInfo.User.GeneralRole > Authentication.User.RoleType.Visitors Then
                ...
                If UserChatInfo.User.GeneralRole >= Authentication.User.RoleType.Visitors Then
                  ...
                  End Select
                End If
                Dim Robot As User = Authentication.User.Load(UserChatInfo.InvitedByRobotName)
                If Robot IsNot Nothing Then
			...
			If HttpContext IsNot Nothing Then
				...
				For Each PageMonitoring As PageMonitor In MonitoringPageVisited
					If PageMonitoring.Page = Page AndAlso PageMonitoring.Archive = Archive AndAlso PageMonitoring.Language = Setting.Language Then
						...
						End If
            Dim AmbitUser As StringCollection = CType(HttpContext.Current.Session("AmbitsChatRobot"), StringCollection)
