...
Namespace WebApplication.Plugin		'Standard namespace obbligatory for all plugins
    ...
    Private Shared Sub Plugin_MasterPageInit(MasterPage As Components.MasterPageEnhanced) Handles Plugin.MasterPageInit
      Dim Configuration As RedirectConfiguration = CType(Plugin.LoadConfiguration(), RedirectConfiguration)
      For Each Redirect As RedirectConfiguration.RedirectSetting In Configuration.Redirects
        Dim IsSpider As Boolean = Extension.IsCrawler(MasterPage.Request())
        If Redirect.Who = RedirectConfiguration.RedirectSetting.WhoRedirect.All OrElse (Redirect.Who = RedirectConfiguration.RedirectSetting.WhoRedirect.Crawlers AndAlso IsSpider) OrElse (Redirect.Who = RedirectConfiguration.RedirectSetting.WhoRedirect.Users AndAlso IsSpider = False) Then
