...
Namespace WebApplication.Plugin   'Standard namespace obbligatory for all plugins
  Public Class ThreeDimensionsGallery
    ...
    Private Shared Sub Plugin_MasterPagePreRender(ByVal MasterPage As Components.MasterPageEnhanced) Handles Plugin.MasterPagePreRender
      Dim Configuration As ThreeDimensionsGalleryConfiguration = CType(Plugin.LoadConfiguration(), ThreeDimensionsGalleryConfiguration)
          ...
          If Not Extension.IsCrawler(MasterPage.Request) OrElse Configuration.ShowToCrawler Then
            If CurrentUser(MasterPage.Session).GeneralRole = Authentication.User.RoleType.Visitors OrElse Configuration.HideToLoggedUser = False Then
              ...
              If (MetaTag Is Nothing OrElse MetaTag.Content <> "noindex") AndAlso MasterPage.ShowRightBar = True Then 'Only page indexabled 
                ...
                If Configuration.IdPhotoAlbum = "" AndAlso Setting.Photoalbums.Count <> 0 Then Configuration.IdPhotoAlbum = Setting.Photoalbums(0)
                Dim Photos = GetRandomPhotos(Configuration.IdPhotoAlbum, 16)
                If CBool(Photos.Count) Then
                  Dim N As Integer = 0
                  Do
