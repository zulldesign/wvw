...
<table border="0" cellspacing="0" cellpadding="3">
  <tr>
	...
	If Request.QueryString = "edit" Or Request.QueryString = "delete" Then
		...
		Set RS = ObjConn.Execute("SELECT ID,Title,ExtLink,InputDate,UserName FROM RSS ORDER BY InputDate")
		If Not(RS.EOF) Then
			...
			Do While Not (RS.EOF)
				...
				End If
				%>
				<form action="adminadd.asp" method=POST>
				<tr bgcolor="<% =BGC %>">
					<td><b><a href="<% =RS("ExtLink") %>" target="new"><% =RS("Title") %></a></b> (<I><% =RS("UserName") %></I>)</td>
	...
	Else
		...
		Else
			...
			Set RS = ObjConn.Execute("SELECT * FROM RSS WHERE ID = " & Request.Form("ID"))
	...
	<tr>
	  ...
	  <td align="left" valign="middle"><input type="text" name="ExtLink" size="40"  maxlength=149 value="<% If IsNull(ExtLink) Then Response.Write(ExtLink) Else Response.Write(Cookdown(ExtLink)) End If %>"></td>
		...
		<tr>
		  <td class="toc" width="150" align="left" valign="top"><b>UserName :</b></td>
		  <td align="left" valign="middle">
	<% Call UserSelect(UserName) %>  
		</td>
	</tr>
	<% Call DateSelect(DropOutDateM,DropOutDateD,DropOutDateY,"DropOutDate","Disappearance Date") %>  
	<tr>
...
End Select%> RSS Entry"> || <input type=reset value="Start Over"></td>
	...
	Else
		...
		Else
			...
			List = "Title,Description,ExtLink,UserName"
		End If
		InsStatement = BuildStatement(List,"Update","RSS","Form") & " WHERE ID = " & Request.Form("ID")
	End If
	...
	Call StaticBuilder(DataDir & "\rblog.mdb","\rss\" & Replace(Request.Form("UserName")," ","_"),Request.Form("UserName"))
	%><tr><td>
	<table width="100%" cellspacing="2" cellpadding="0" border="0" class="bodytext1">
	<tr>
	  <td class="toc" width="150" align="left" valign="top"><b>Title :</b></td>
	...
	If Request.Form("action") = "delete" And IsNumeric(Request.Form("ID")) Then
		...
		InsStatement = "DELETE FROM RSS WHERE ID = " & Request.Form("ID")
		Set qryInsert = InsConn.Execute(InsStatement)
