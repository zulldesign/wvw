...
<div id="container">
   ...
   <!-- #include virtual="config.inc" -->
    ...
    <table id="tbnews" width="660" cellpadding="15">
      ...
      <div class="fb-like" data-href="<%= sn %>/blogdotnet/displaycomm1.aspx?idn=<%= mdrs["idn"] %>" data-send="false" data-layout="button_count" data-width="250" data-show-faces="true"></div>
       ...
       <tr><td><%
            ...
            mdrs.Close();
            mc.Close();
            Session["idnpag"] = Request.QueryString["idn"];
            ...
            int nrpag = drcpag.Count;
            int p;
            int plim = (int)nrpag / 5;
            if (plim != 0)
            {
                for (p = 1; p <= plim + 1; p++)
                {
            %>
            <a id="colleg"  href="displaycomm2.aspx?id=<%= p %>&idn=<%= Session["idnpag"] %>"><%= p%></a>|
          <%  
            }
            }              
                     %></td></tr>
